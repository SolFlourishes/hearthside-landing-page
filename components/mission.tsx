export function Mission() {
  ...
  return (
    <section aria-labelledby="mission-heading" className="py-8 bg-white">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          {phases.map((phase, index) => (
            <article
              ...
              aria-describedby={`phase-${index}-description`}
            >
              {/* Icon */}
              <div
              ...
              {/* Content */}
              <h3 id={`phase-${index}-title`} className="font-serif text-lg font-bold text-[#1F2937] mb-1">
                {phase.title}
              </h3>
              <p className="text-xs font-semibold text-[#007B8C] mb-2 uppercase tracking-wide">{phase.subtitle}</p>
              <p id={`phase-${index}-description`} className="text-[#6B7280] leading-relaxed text-sm">
                {phase.description}
              </p>
