...
export function Footer() {
  ...
  return (
    <footer role="contentinfo" className="bg-[#1F2937] dark:bg-[#111827] text-white py-8">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <section aria-labelledby="newsletter-heading" className="mb-6 pb-6 border-b border-white/10">
          <div className="max-w-2xl mx-auto text-center">
            <h3 id="newsletter-heading" className="font-serif text-xl font-bold mb-2">
              Stay Connected
            </h3>
            ...
            >
              ...
              <Button
                ...
                aria-label={subscribeStatus === "loading" ? "Subscribing..." : "Subscribe to newsletter"}
              >
                {subscribeStatus === "loading" ? "..." : subscribeStatus === "success" ? "✓" : "Subscribe"}
              </Button>
            </form>
            {subscribeStatus === "success" && (
              <p id="subscribe-success" className="text-[#FFC72C] text-sm mt-2" role="status" aria-live="polite">
                Thank you for subscribing!
              </p>
            )}
            {subscribeStatus === "error" && (
              <p id="subscribe-error" className="text-red-400 text-sm mt-2" role="alert" aria-live="assertive">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-6 gap-8 mb-6">
          ...
          {Object.entries(footerLinks).map(([category, links]) => (
            <nav key={category} aria-labelledby={`footer-${category.toLowerCase()}-heading`}>
              <h4 id={`footer-${category.toLowerCase()}-heading`} className="font-semibold text-white mb-3 text-sm">
                {category}
              </h4>
